 ...
 */
public class AutoDownUpScrollViewPager extends ViewPager {
    private static final String TAG = "AutoDownUpScrollViewPag";
    ...
    public AutoDownUpScrollViewPager(Context paramContext) {
        super(paramContext);
        init();
    }

    public AutoDownUpScrollViewPager(Context paramContext, AttributeSet paramAttributeSet) {
...
/* ////////------ 大海注释的 2016/10/31  ------//////////   */
        ...
        if (ev.getAction() == MotionEvent.ACTION_MOVE) {
            scrollY = ev.getY();
            scrollX = ev.getX();
//            if (Math.abs(culY - scrollY) > 20 && scrollX == culX) {
//                getParent().requestDisallowInterceptTouchEvent(true);
//            } else {
//                getParent().requestDisallowInterceptTouchEvent(false);
//                this.startAutoScroll();
//            }
            //解决viewpager与scroll的冲突
            //当滑动X的距离大于Y的距离时,viewpager处理事件,否则给父控件scroll处理事件
                if(Math.abs(culY - scrollY)< Math.abs(culX - scrollX)){
                    getParent().requestDisallowInterceptTouchEvent(true);
                }else{
                    getParent().requestDisallowInterceptTouchEvent(false);
                    this.startAutoScroll();
...
/* ////////------ 大海加上的 2016/10/31 ------//////////   */
    ...
    private float scrollX = 0f, culX=0f,scrollY = 0f, culY = 0f;
