 ...
 * @date 18/11/19
 */
@Route(path= ActivityContracts.ACTIVITY_NOTICE)
public class NoticeActivity extends BaseActivity {
    @BindView(R.id.listView)
    RecyclerView recyclerView;
    @Override
    protected void initView() {
        mBaseToolBar.setToolbarTitle("超次元");
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        recyclerView.addItemDecoration(new DividerItemDecoration(mContext,DividerItemDecoration.VERTICAL));
        recyclerView.setAdapter(new MyAdapter(mContext,3));
    }
    ...
    private class MyAdapter extends RecyclerView.Adapter<MyAdapter.MyViewHolder> {
        ...
        public MyAdapter.MyViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
            View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.listitem_notice, viewGroup, false);
